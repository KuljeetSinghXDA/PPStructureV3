...
# Install protobuf prerequisite (ensures compatible version for build)
RUN pip install protobuf==3.20.3

# Clone Paddle with all submodules (ensures complete dependencies)
RUN git clone --recurse-submodules https://github.com/PaddlePaddle/Paddle.git /Paddle && \
    cd /Paddle && \
    git checkout v3.2.0

# Disable -Werror warnings safely (converts errors to warnings for ARM compatibility)
