...
# Latest GL/GUI libs commonly required by CV backends used by PaddleOCR, plus poppler-utils for pdf2image
...
RUN python -m pip install --no-cache-dir -U pip --root-user-action=ignore \
 ...
 && python -m pip install --no-cache-dir "paddleocr[all]" fastapi uvicorn[standard] python-multipart pdf2image --root-user-action=ignore
