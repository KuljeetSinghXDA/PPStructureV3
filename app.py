...
app = FastAPI(
    ...
    version="1.3.0",
    description="FastAPI wrapper around PaddleOCR PP-StructureV3 with full parameter exposure, JSON and Markdown outputs, and PDF Markdown concatenation. Tuned defaults for medical lab reports."
)

# -------------------------
# Options model (defaults chosen for CPU + medical lab report accuracy)
# -------------------------
class ParseOptions(BaseModel):
    ...
    # Device & perf
    device: str = "cpu"
    enable_mkldnn: Optional[bool] = False      # safer off by default on ARM64
    mkldnn_cache_capacity: Optional[int] = 5
    cpu_threads: Optional[int] = 4

    # Language
    lang: Optional[str] = "en"

    # Module toggles (exposed; pass only if not None)
    ...
    # Note: some builds use 'use_chart_recognition' (not 'use_chart_parsing'); we keep your field
    # and only pass it if the current installed signature exposes it.
    use_chart_parsing: Optional[bool] = False
    ...
    text_det_limit_side_len: Optional[int] = 1536   # higher side length for small fonts
    ...
    text_rec_score_thresh: Optional[float] = 0.60  # filter low-confidence text

    # Table, formula, seal, chart model names/dirs (exposed; pass only if provided)
...
def _opts_to_init_kwargs(opts: ParseOptions) -> Dict[str, Any]:
    ...
    # Map potentially versioned names (chart_recognition vs chart_parsing)
    chart_toggle_key = "use_chart_recognition" if "use_chart_recognition" in allowed else (
        "use_chart_parsing" if "use_chart_parsing" in allowed else None
    )
    chart_model_key = "chart_recognition_model_name" if "chart_recognition_model_name" in allowed else (
        "chart_parsing_model_name" if "chart_parsing_model_name" in allowed else None
    )
    chart_model_dir_key = "chart_recognition_model_dir" if "chart_recognition_model_dir" in allowed else (
        "chart_parsing_model_dir" if "chart_parsing_model_dir" in allowed else None
    )
    ...
    candidate: Dict[str, Any] = {
        ...
        # Chart toggle uses whichever key the installed version exposes
        chart_toggle_key: (opts.use_chart_parsing if chart_toggle_key else None),
        ...
        # Chart models (version-aware keys)
        chart_model_key: (opts.chart_parsing_model_name if chart_model_key else None),
        chart_model_dir_key: (opts.chart_parsing_model_dir if chart_model_dir_key else None),
    }

    # Drop None and keys not supported by the installed version
    init_kwargs = {k: v for k, v in candidate.items() if (k and (v is not None) and (k in allowed))}
    return init_kwargs
...
def _warmup(pipeline: PPStructureV3) -> None:
    # Warm up with a real PNG file path (safer than ndarray on some stacks)
    ...
    with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tf:
        ...
        tmp_path = tf.name
    try:
        _ = pipeline.predict(input=tmp_path)
    except Exception:
        pass
    finally:
        try:
...
def index():
    return {
        ...
        "defaults": {
            ...
            "cpu_threads": DEFAULT_OPTS.cpu_threads,
            "enable_mkldnn": DEFAULT_OPTS.enable_mkldnn,
            "lang": DEFAULT_OPTS.lang,
            "layout_detection_model_name": DEFAULT_OPTS.layout_detection_model_name,
            "text_detection_model_name": DEFAULT_OPTS.text_detection_model_name,
            "text_recognition_model_name": DEFAULT_OPTS.text_recognition_model_name,
            "use_table_recognition": DEFAULT_OPTS.use_table_recognition,
            "use_textline_orientation": DEFAULT_OPTS.use_textline_orientation,
            "use_doc_orientation_classify": DEFAULT_OPTS.use_doc_orientation_classify,
            "text_det_limit_side_len": DEFAULT_OPTS.text_det_limit_side_len,
            "text_rec_score_thresh": DEFAULT_OPTS.text_rec_score_thresh
        },
        "endpoints": {
            "POST /parse": "Upload multi-file (images or PDFs). Returns JSON + Markdown; concatenates PDF Markdown."
        },
        "notes": [
            "Parameters are passed only if supported by the installed PP-StructureV3 version (runtime introspection).",
            "Safer warmup via file path helps avoid native crashes on some ARM64 stacks.",
            "Defaults tuned for table-heavy medical lab reports on CPU."
        ]
    }
